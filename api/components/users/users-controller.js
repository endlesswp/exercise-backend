...
async function createUser(request, response, next) {
  try {
    ...
    if (password !== password_confirm) {
      const errorResponse = {
        statusCode: errorTypes.INVALID_PASSWORD.status,
        error: errorTypes.INVALID_PASSWORD.code,
        description: errorTypes.INVALID_PASSWORD.description,
...
async function updateUser(request, response, next) {
  try {
    ...
    const checkEmail = await usersService.checkEmail(email, id);
    if (checkEmail) {
      throw errorResponder(
        errorTypes.EMAIL_ALREADY_TAKEN,
